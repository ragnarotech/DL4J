...
public class FXMLStartController implements Initializable {
    ...
    private void loadProject(ActionEvent event) {
		...
		model = loadNetwork("lenetmnist.zip");
		//model = loadNetwork("mnist-model.zip");
		//ModelSerializer.restoreMultiLayerNetwork(new File("mnist-model.zip"));
	...
	public MultiLayerNetwork loadNetwork(String resourcePath) throws Exception{
		return MultiLayerNetwork.load(new File(resourcePath),false);
	}


	public String testImage(BufferedImage img) throws Exception {
		INDArray input = loader.asMatrix(img).reshape(1,784);

		INDArray output = model.output(input);
		String putStr = output.toString();
		return "Prediction: " + model.predict(input)[0] + ", Percents: " + putStr;
	}
